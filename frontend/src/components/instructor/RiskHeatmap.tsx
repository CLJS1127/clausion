...
const MOCK_STUDENTS: HeatmapStudent[] = [
  { id: 1, name: '김민준', initial: '김', risk: 'safe' },
  { id: 2, name: '이서연', initial: '이', risk: 'safe' },
  { id: 3, name: '박지호', initial: '박', risk: 'caution' },
  { id: 4, name: '최수빈', initial: '최', risk: 'safe' },
  { id: 5, name: '정예린', initial: '정', risk: 'danger' },
];
...
export default function RiskHeatmap() {
  ...
  const { data: students = MOCK_STUDENTS } = useQuery({
    ...
    queryFn: async () => {
      if (!courseId) return MOCK_STUDENTS;
      const entries = await instructorApi.getCourseHeatmap(courseId);
      return entries.length > 0 ? entries.map(mapEntry) : MOCK_STUDENTS;
    },
